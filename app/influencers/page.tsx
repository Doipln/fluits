...
import { Check, MapPin, Users, Search, HeartIcon, Grid3X3, List, SlidersHorizontal, ChevronDown } from "lucide-react"
import { Drawer, DrawerContent, DrawerFooter, DrawerClose, DrawerHeader, DrawerTitle } from "@/components/ui/drawer"
import { Progress } from "@/components/ui/progress"
import { calculateInfluencerProfileCompletion } from "@/lib/profile-completion"
import { allInfluencers } from "@/lib/influencer-store"

const recommendedInfluencers = allInfluencers.slice(0, 4)
...
export default function InfluencersPage() {
  ...
  const [filteredInfluencers, setFilteredInfluencers] = useState(allInfluencers)
  ...
  useEffect(() => {
    ...
    if (influencerMode) {
      setProfileCompletion(calculateInfluencerProfileCompletion())
    }
  }, [])
  ...
  useEffect(() => {
    const handleFocus = () => {
      const influencerMode = localStorage.getItem("influencer_mode") === "true"
      if (influencerMode) {
        const newCompletion = calculateInfluencerProfileCompletion()
  ...
  useEffect(() => {
    const influencerMode = localStorage.getItem("influencer_mode") === "true"
    let displayInfluencers = allInfluencers

    if (influencerMode) {
      const username = localStorage.getItem("username") || localStorage.getItem("influencer_nickname") || "내 프로필"
      const avatar = localStorage.getItem("user_avatar") || "/placeholder.svg"
      const category = localStorage.getItem("influencer_category") || "뷰티"
      const region = localStorage.getItem("influencer_broad_region") || "서울"
      const bio = localStorage.getItem("influencer_bio") || "프로필 소개를 입력해주세요"
      const instagramId = localStorage.getItem("influencer_instagram_id") || ""
      const hashtags = JSON.parse(localStorage.getItem("influencer_profile_hashtags") || "[]")
      ...
      const filteredInfluencers = allInfluencers.filter((inf) => inf.id !== 999)
      displayInfluencers = [currentUserProfile, ...filteredInfluencers]
    }

    const filtered = displayInfluencers
      .filter((influencer) => {
  ...
  }, [selectedCategories, selectedFollowerRanges, searchQuery, sortBy])
        ...
        <div className="space-y-3">
          <div>
            <div className="flex justify-between items-center">
              <h2 className="text-lg font-bold text-gray-900">플루잇이 추천하는 파트너</h2>
            ...
            <div className="flex justify-between items-center gap-1">
              ...
              <div className="flex-shrink-0">
                <div className="relative flex rounded-full bg-gray-100 overflow-hidden h-8 p-1">
                  <div
                    className={`absolute top-1 bottom-1 w-[calc(50%-4px)] bg-white rounded-full transition-all duration-300 ease-in-out shadow-sm ${viewMode === "grid" ? "left-1" : "left-[calc(50%+2px)]"}`}
                  />

                  <button
                    className={`relative z-10 rounded-full px-2 py-1 h-full flex-1 transition-all duration-200 flex items-center justify-center ${viewMode === "grid" ? "text-gray-900" : "text-gray-500 hover:text-gray-700"}`}
                  ...
                  <button
                    className={`relative z-10 rounded-full px-2 py-1 h-full flex-1 transition-all duration-200 flex items-center justify-center ${viewMode === "horizontal" ? "text-gray-900" : "text-gray-500 hover:text-gray-700"}`}
