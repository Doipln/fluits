...
export interface Campaign {
  ...
  visitTimeInfo?: string
  preferentialInfo?: string
  isBrandLocationRequired?: boolean
  isHiddenByUser?: boolean // Added field to track user-hidden campaigns
  platform?: "instagram" | "blog" // Added platform field
}
...
interface CampaignContextType {
  ...
  updateCampaignStatus: (id: number, status: "진행중" | "마감" | "비공개 글") => void
  ...
  hideCampaignForUser: (id: number) => void // Added function to hide campaigns
}
...
const initialCampaigns: Campaign[] = [
  {
    ...
    visitType: "visit",
    applicantList: [],
    visitTimeInfo: "평일 오전 10시 ~ 오후 6시 사이 방문 가능\n주말 및 공휴일 방문 불가",
    preferentialInfo: "뷰티 관련 콘텐츠 제작 경험이 많으신 분 우대\n건성 피부 고민을 가진 분 우대",
    isBrandLocationRequired: false,
    platform: "instagram", // Added platform field
  },
  {
    id: 2,
    title: "신상 립스틱 리뷰어 모집",
    ...
    visitType: "visit",
    applicantList: [],
    visitTimeInfo: "평일 오후 2시 ~ 오후 8시 사이 방문 가능\n토요일 오전 11시 ~ 오후 5시 방문 가능",
    preferentialInfo: "메이크업 아티스트 자격증 보유자 우대\nSNS 팔로워 3만명 이상 우대",
    isBrandLocationRequired: false,
    platform: "instagram", // Added platform field
  },
  {
    ...
    visitType: "visit",
    applicantList: [],
    visitTimeInfo: "평일 오전 9시 ~ 오후 7시 사이 방문 가능\n주말 방문 가능 (사전 예약 필수)",
    preferentialInfo: "헬스 트레이너 자격증 보유자 우대\n피트니스 관련 콘텐츠 제작 경험 1년 이상 우대",
    isBrandLocationRequired: false,
    platform: "instagram", // Added platform field
  },
  {
    ...
    visitType: "visit",
    applicantList: [],
    visitTimeInfo: "평일 오전 6시 ~ 오후 10시 사이 방문 가능\n주말 오전 8시 ~ 오후 8시 방문 가능",
    preferentialInfo: "헬스 트레이너 자격증 보유자 우대\n피트니스 관련 콘텐츠 제작 경험 1년 이상 우대",
    isBrandLocationRequired: false,
    platform: "instagram", // Added platform field
  },
  {
    ...
    applicantList: [],
    visitTimeInfo: "평일 오전 10시 ~ 오후 6시 사이 방문 가능\n반려동물 동반 필수",
    preferentialInfo: "반려동물 양육 경험 3년 이상 우대\n펫 관련 자격증 보유자 우대",
    isBrandLocationRequired: false,
    platform: "instagram", // Added platform field
  },
  {
    ...
    visitType: "visit",
    applicantList: [],
    visitTimeInfo: "평일 오후 1시 ~ 오후 7시 사이 방문 가능\n주말 방문 불가",
    preferentialInfo: "테크 관련 자격증 보유자 우대\n언박싱 영상 제작 경험이 있는 분 우대",
    isBrandLocationRequired: false,
    platform: "instagram", // Added platform field
  },
]
...
export function CampaignProvider({ children }: { children: ReactNode }) {
    ...
    if (typeof window !== "undefined") {
      try {
        ...
        if (savedCampaigns) {
          ...
          return [...userCampaigns, ...initialCampaigns]
        }
      } catch (e) {
        console.error("[v0] Failed to parse saved campaigns:", e)
      }
    }
  ...
  useEffect(() => {
    if (mounted && typeof window !== "undefined") {
      try {
        const userCampaigns = campaigns.filter((c) => c.isUserCreated)
        localStorage.setItem("user_campaigns", JSON.stringify(userCampaigns))
        console.log("[v0] Saved user campaigns to localStorage:", userCampaigns.length)
      } catch (e) {
        console.error("[v0] Failed to save campaigns to localStorage:", e)
      }
    }
  }, [campaigns, mounted])
  ...
  const addCampaign = (newCampaign: Omit<Campaign, "id" | "timeAgo" | "comments" | "likes" | "views">) => {
    const campaign: Campaign = {
      ...
      visitTimeInfo: newCampaign.visitTimeInfo,
      preferentialInfo: newCampaign.preferentialInfo,
      isBrandLocationRequired: newCampaign.isBrandLocationRequired || false,
      platform: newCampaign.platform || "instagram", // Added platform field
    }
    console.log("[v0] Adding campaign with ID:", campaign.id)
  ...
  const updateCampaignStatus = (id: number, status: "진행중" | "마감" | "비공개 글") => {
      ...
      prev.map((campaign) =>
        campaign.id === id
          ? {
              ...
              visitTimeInfo: updates.visitTimeInfo || campaign.visitTimeInfo,
              preferentialInfo: updates.preferentialInfo || campaign.preferentialInfo,
              isBrandLocationRequired: updates.isBrandLocationRequired || campaign.isBrandLocationRequired,
              platform: updates.platform || campaign.platform, // Added platform field
            }
          : campaign,
      ),
    )
  ...
  const hideCampaignForUser = (id: number) => {
    setCampaigns((prev) =>
      prev.map((campaign) => (campaign.id === id ? { ...campaign, isHiddenByUser: true } : campaign)),
    )
    console.log("[v0] Campaign hidden by user:", id)
  }
  ...
  return (
    <CampaignContext.Provider
      value={{
        ...
        deleteCampaign,
        updateApplicantStatus,
        hideCampaignForUser, // Added to context
      }}
    >
      {children}
    </CampaignContext.Provider>
